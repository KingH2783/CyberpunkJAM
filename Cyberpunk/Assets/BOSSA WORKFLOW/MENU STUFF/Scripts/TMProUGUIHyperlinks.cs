...
{
    [SerializeField] private bool setDefaultColor;
    [SerializeField]
    private Color32 defaultColor = new Color32(0x00, 0x59, 0xFF, 0xFF);
    [SerializeField]
    ...
    private void Update()
    {
        if(previousLinkCount != textMeshPro.textInfo.linkCount)
        {
            previousLinkCount = textMeshPro.textInfo.linkCount;
            if (setDefaultColor)
            {
                for (int i = 0; i < textMeshPro.textInfo.linkCount; i++)
                {
                    SetLinkColor(i, defaultColor);
                }
            }
        }
    }
    ...
    public void OnPointerUp(PointerEventData eventData)
    {
        ...
        if (linkIndex != -1 && linkIndex == pressedLinkIndex) // Was pointer intersecting the same link as OnPointerDown?
        {
            ...
            SetLinkColor(linkIndex, hoveredColor);
    ...
    {
        return TMP_TextUtilities.FindIntersectingLink(textMeshPro, Mouse.current.position.ReadValue(), mainCamera);
    }
